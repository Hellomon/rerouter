...
export class Utils {
  ...
  public static startApp(packageName: string) {
    ...
    execute(`ANDROID_DATA=/data monkey --pct-syskeys 0 -p ${packageName} -c android.intent.category.LAUNCHER 1 || true`);
    execute(
      'ANDROID_BOOTLOGO=1 ' +
      'ANDROID_ROOT=/system ' +
      ...
      `monkey --pct-syskeys 0 -p ${packageName} -c android.intent.category.LAUNCHER 1 || true`
    );
    // android 12
    execute(
      'ANDROID_BOOTLOGO=1 ' +
